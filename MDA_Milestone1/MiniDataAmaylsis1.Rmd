...
<!-------------------------- Start your work below ---------------------------->
I am choosing to work with the apt_buildings dataset. I chose this one for its combination of both double vectors and numberic vectors. I also like that it has variables that are binary (yes/no) as well as ones that are more descriptive. 
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
Is there a relationship between the year the apartment was built and the number of storeys in the building? 
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
1. Plot the distribution of a numeric variable. Chose this exercise to understand the distribution of the number of storeys in the apartment buildings.

```{r}
storeys <- ggplot(apt_buildings, aes(x=no_of_storeys)) + geom_histogram(binwidth = 1)
```

Here I'm creating a histogram of the number of storeys in the apartment buildings. I also adjusted bin width to see each storey number. 

```{r} 
print(storeys)
```

2. Explore the relationship between 2 variables in a plot. I would like to use this exercise to explore the relationship between the year the appartment was built and the number of storeys that it contains to start answering the research question listed in Exercise 1.4. 

```{r}
StoreysXYearBuilt <- ggplot(apt_buildings, aes(x = year_built, y = no_of_storeys)) + geom_point()
```

Here I am plotting the relationship between the year the apartment was built and the number of storeys in the apartment. 

```{r}
print(StoreysXYearBuilt)
```

3. Make a new tibble with a subset of your data, with variables and observations that you are interested in exploring. I am doing this to remove variables I am uninterested in (addresses, property manager, etc.).
...
Here I am creating a tibble with my desired variables, including all observations for each of them. 

```{r}
print(apt_build2.0)
```

4. Filter observations in your data according to your own criteria. Filtering to only see apartments built after 1950 to use for future research questions. 

```{r}
apt_1950 <- filter(apt_buildings, year_built >1950)
```

Here I am filtering to only show apartment buildings that were built after 1950.
